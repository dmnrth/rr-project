...
foreach (i = 1:n_splits) %dopar% {
  ...
  saveRDS(arima_garch_forecasts,
          file = paste0("output/arima-", tolower(g_model), "_", distribution,
                        "_", window_len, "/", split_start_date, "_", split_end_date ,".rds"))
}
...
for (i in 1:length(window_len)){
  
  forecast_indexes <- index(sp500_prices[(which(index(sp500_prices) == "2002-01-02") + window_len[i]):length(sp500_prices)]) 
  ...
  foreach (i = 1:n_splits) %dopar% {
    ...
    arima_garch_forecasts <- arima_garch_rolling_forecast(sp500_prices,
                                                        ...
                                                        estimation_end_date = paste0(split_end_date),
                                                        progress_bar = TRUE,
                                                        estimation_window_length = window_len[i],
                                                        g_model = g_model,
                                                        distribution = distribution)
    
    saveRDS(arima_garch_forecasts,
            file = paste0("output/arima-", tolower(g_model), "_", distribution,
                        "_", window_len[i], "/", split_start_date, "_", split_end_date ,".rds"))
  }
}
stopCluster(cl)
```
...
for (i in 1:length(distribution)){
  ...
  foreach (i = 1:n_splits) %dopar% {
    ...
    saveRDS(arima_garch_forecasts,
           file = paste0("output/arima-", tolower(g_model), "_", distribution[i],
                        "_", window_len, "/", split_start_date, "_", split_end_date ,".rds"))
  }
}
stopCluster(cl)
